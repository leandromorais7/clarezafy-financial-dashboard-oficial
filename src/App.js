...
const translations = {
    pt: {
        ...
        supportDeveloper: "Pague um café para o dev",
        pixCopied: "Pix copiado! Obrigado ☕",
        copyPix: "Copiar chave Pix",
        copyError: "Erro ao copiar. Copie manualmente: ",
    },
    en: {
        title: "ClarezaFy",
        ...
        supportDeveloper: "Buy the dev a coffee",
        pixCopied: "Pix copied! Thanks ☕",
        copyPix: "Copy Pix key",
        copyError: "Error copying. Copy manually: ",
    }
};
...
const CoffeeIcon = () => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5 mr-2"><path d="M18 8h1a4 4 0 0 1 0 8h-1"></path><path d="M2 8h16v9a4 4 0 0 1-4 4H6a4 4 0 0 1-4-4V8z"></path><line x1="6" y1="1" x2="6" y2="4"></line><line x1="10" y1="1" x2="10" y2="4"></line><line x1="14" y1="1" x2="14" y2="4"></line></svg>;
...
export default function App() {
    // --- IMPORTANTE: SUBSTITUA SUA CHAVE PIX AQUI ---
    const CHAVE_PIX = "leandromoraisneves@hotmail.com"; 
    // ------------------------------------------------
    ...
    // --- FUNÇÃO DE COPIA COM FALLBACK ---
    const handleCopyPix = () => {
        const textToCopy = CHAVE_PIX;
        
        // Função auxiliar para fallback
        const fallbackCopy = () => {
             const textArea = document.createElement("textarea");
             textArea.value = textToCopy;
             textArea.style.position = "fixed"; // Evita rolagem da tela
    ...
    return (
        <div className="bg-slate-50 dark:bg-slate-900 min-h-screen font-sans text-gray-800 dark:text-gray-200 p-4 sm:p-6 lg:p-8 overflow-x-hidden">
            ...
            {/* Correção para rolagem lateral: style ajustado para esconder o template de forma mais segura sem quebrar o HTML2Canvas */}
            <div style={{ position: 'absolute', width: 0, height: 0, overflow: 'hidden', left: 0, top: 0 }}>
                <ReportTemplate data={dadosMesAtual} t={t} formatCurrency={formatCurrency} formatDateDisplay={formatDateDisplay} chartData={chartData} theme={theme} resultado={resultado} custoTotal={custoTotal} totalCustosFixos={totalCustosFixos} totalCustosVariaveis={totalCustosVariaveis} margemDeLucro={margemDeLucro} pontoDeEquilibrio={pontoDeEquilibrio} />
            </div>
            <div className="max-w-6xl mx-auto flex flex-col min-h-screen">
                ...
                <footer className="text-center mt-12 py-6 border-t border-slate-200 dark:border-slate-700 flex flex-col items-center">
                    <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
                        &copy; {new Date().getFullYear()} {t('footerText')}
                    </p>
                    
                    {/* Botão de Apoie o Desenvolvedor */}
                    <button 
                        onClick={handleCopyPix}
                        className={`flex items-center px-4 py-2 rounded-full font-semibold transition-all duration-300 transform hover:scale-105 active:scale-95 shadow-md ${pixCopied ? 'bg-green-500 text-white' : 'bg-yellow-400 text-yellow-900 hover:bg-yellow-300'}`}
                    >
                        <CoffeeIcon />
                        {pixCopied ? t('pixCopied') : t('supportDeveloper')}
                    </button>
                    {pixCopied && <span className="text-xs text-green-600 dark:text-green-400 mt-2 fade-in">{t('copyPix')}</span>}
                </footer>
            </div>
        </div>
    );
